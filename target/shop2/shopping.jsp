...
<div id="header" class="wrap">
	...
	<div class="help"><c:if test="${name!=null}"><a href="selectdd?dd=${name.EU_USER_ID }">个人订单</a></c:if><c:if test="${name!=null}">当前用户${name.EU_USER_ID }</c:if><a href="ShopSelect" class="shopping">购物车</a><c:if test="${name==null}"><a href="login.jsp">登录</a>|<a href="register.jsp">注册</a></c:if><c:if test="${name!=null}"><a href="zx">退出</a></c:if><a href="${pageContext.request.contextPath }/SelallServlet">留言</a><c:if test="${name.EU_STATUS==2}"><a href="manage/index.jsp" >去后台</a></c:if></div>
	<div class="navbar">
		<ul class="clearfix">
			<c:choose>
...
<div id="position" class="wrap">
	您现在的位置：<a href="index.html">跳蚤市场-商城</a> &gt; 购物车
</div>
<div class="wrap">
	<div id="shopping">
		<form action="gmServlet">
			...
			<table>
				...
				<c:forEach var="rs" items="${requestScope.shoplist}">
					<tr id="product_id_1">
						<td class="thumb">
                            <input type="checkbox" name="op"  onclick="dxcheck(), changeState(this)" />
                            <input type="hidden" name="isHideInMenu" value="false"/>

                            <img src="images/product/${rs.es_ep_file_name }" height="100" width="100" />

						<a href="selectProductView?id=${rs.es_ep_id}">${rs.es_ep_name }</a></td>

						<td class="price" id="price_id_1">
							<span id="priceText" name="priceText" >${rs.es_ep_price}</span>
                            <!-- 商品单价 --><input type="hidden" name="sPPrice" value="${rs.es_ep_price}" />
			...
			<div class="button" >
                <input style="display:none;" type="submit"  value="" id="jsbutton" /></div>
		</form>
	</div>
	<script type="text/javascript">
